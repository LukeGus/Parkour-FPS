    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        public class Events // Store your events
        {
            public UnityEvent OnMove, OnJump, OnLand, OnCrouch, OnStopCrouch, OnSprint, OnSpawn, OnSlide, OnStartWallRun, OnStopWallRun,
                                OnWallBounce, OnEndClimb, OnStartGrapple, OnGrappling, OnStopGrapple, OnGrappleEnabled;
        }
        [System.Serializable]
        ...
        [Tooltip("The camera component that weapons are rendered on.")]
        public Camera weaponCamera;

        [Tooltip("The main camera component used.")]
       ...
       "       if false: Speed while shooting = walkSpeed")]
        ...
        private void Update()
        {
            ...
            if (InputManager.jumping && wallOpposite && canWallBounce && CheckHeight()) WallBounce();

            if (canWallRun)
            {
                CheckWalls();
                if (InputManager.yMovementActioned && (wallLeft || wallRight) && CheckHeight() &&
                    rb.velocity.magnitude > walkSpeed && !grounded) WallRun();
                else StopWallRun();
            }
        }
        public void HandleVelocities()
        {
            ...
            {
                bool shouldRun;
                if (!canRunBackwards && InputManager.y < 0 || (!canRunWhileShooting && InputManager.shooting) || !canRunSideways && InputManager.x != 0 && InputManager.y == 0)
                {
        ...
        public void Movement(bool move)
        {
            ...
            rb.AddForce(orientation.transform.forward * InputManager.y * acceleration * Time.deltaTime * multiplier * multiplierV);
        ...
        public void Look()
        {
            ...
            float mouseX = (InputManager.mousex * InputManager.sensitivity_x * Time.fixedDeltaTime + InputManager.controllerx * InputManager.controllerSensitivityX * Time.fixedDeltaTime) * inverted;
            float mouseY = (InputManager.mousey * InputManager.sensitivity_y * Time.fixedDeltaTime * inverted + InputManager.controllery * InputManager.controllerSensitivityY * Time.fixedDeltaTime * -inverted);
            ...
            if (!applyAimAssist || target == null || Vector3.Distance(target.position, transform.position) > maximumDistanceToAssistAim) return;
        ...
        private void StartGrapple()
        {
            ...
            if (Physics.Raycast(weaponCamera.transform.position, weaponCamera.transform.forward, out hit, maxGrappleDistance, whatIsGround))
            {
                grappling = true;
